...
public class ArticlePopulateData {
    ...
    public ArticlePopulateData(List<String> authors) {
        this.authors = authors;
    }

    public List<Article> populate() {
        List<Article> articles = new ArrayList<>();
        articles.add(articleFactory.create("ABTS11", authors.get(1), Arrays.asList("Politic", "Thriller")));
        articles.add(articleFactory.create("BTSGA11", authors.get(0), Arrays.asList("Horror")));
        articles.add(articleFactory.create("TATA12", "Zatty", Arrays.asList("Emotions","Horror","West")));
